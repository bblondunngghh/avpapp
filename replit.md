...
- July 3, 2025: CRITICAL BUG FIX - Fixed text notification delivery failures in assistance center by converting HTML emails to plain text format for T-Mobile SMS gateway compatibility. The issue was that T-Mobile's email-to-SMS gateway `5127755739@tmomail.net` was rejecting HTML content, causing bounce-back notifications to `brandon@accessvaletparking.com`. Now automatically detects SMS gateways and sends plain text messages for reliable delivery.
