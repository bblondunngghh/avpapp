...
- July 3, 2025: CRITICAL BUG FIX - Fixed text notification delivery failures in assistance center by converting HTML emails to plain text format for T-Mobile SMS gateway compatibility. The issue was that T-Mobile's email-to-SMS gateway `5127755739@tmomail.net` was rejecting HTML content, causing bounce-back notifications to `brandon@accessvaletparking.com`. Solution: (1) Updated email service to automatically detect SMS gateways and send plain text messages, (2) Switched all 4 restaurant locations from problematic `@tmomail.net` to working `@msg.fi.google.com` gateway. Text notifications now deliver successfully without bounce-backs.
